...
void setStarttime() { startTime = (time(NULL)) * 1000; }
...
int read_command(char **com, char ***par) {
    ...
    *com = (*par)[0];
    return background;
    char *paramerters = strtok(line, " ");
    int i = 0;
    while (paramerters != NULL) {
        (*par)[i] = (char *)malloc(strlen(paramerters) + 1);
        strcpy((*par)[i], paramerters);
        paramerters = strtok(NULL, " ");
        i++;
    }
    // Set the command
    *com = (char *)malloc(strlen((*par)[0]) + 1);
    strcpy(*com, (*par)[0]);
    // Free the memory
    free(line);
    return background;
